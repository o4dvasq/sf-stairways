...
**`SeedDataService.seedIfNeeded()` and `target_list.json` are no longer called at launch.** The function loaded a bundled JSON of Oscar's personal walk history and inserted it as `WalkRecord` entries for any user whose CloudKit private database was empty — i.e., every new test user. This caused test users to see Oscar's 8 walked stairways pre-marked on first launch. The original purpose was single-user bootstrapping before CloudKit was set up. Now that the app is multi-user, each user must start with an empty walk history. Oscar's data is safe in his CloudKit private database and was never at risk. The `SeedStairway` struct, `seedIfNeeded()` method, and `hasSeededKey` constant were all deleted from `SeedDataService.swift`.
