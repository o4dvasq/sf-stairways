...
- `ListTab` — searchable, filterable stairway list (All/Walked); tap row → `StairwayBottomSheet` sheet; queries `StairwayOverride` and passes to each row; section headers use `.system(.subheadline, design: .rounded)`
- `ProgressTab` (tab label: "Stats") — completion ring (stroke: `brandOrange`, not `walkedGreen`; number: SF Pro Rounded size 36); stats grid via `StatCard` (background: `surfaceCardElevated`; value/label font: `.system(.title3/.caption, design: .rounded)`); **collapsible neighborhood `DisclosureGroup` rows** (default collapsed; expand to show walked stairways with name, steps, date — sorted most recent first; no elevation); toolbar has sync icon + gear icon; height stat uses `resolvedHeightFt`
