...
- `MapTab` — MapKit full-screen map (dark appearance), `brandOrange` top bar with leading gear (settings) and trailing Around Me + Tag Filter; filter pills (All/Walked/Nearby); floating `ProgressCard` (bottom-right, 120pt wide) with amber accent bar header + floating search circle above it; no Saved filter; launches at default city-wide view (latDelta ~0.06, centered on SF) — no auto-zoom on launch; tracks `mapSpan` via `.onMapCameraChange(frequency: .continuous)` and passes a lerped `pinScale` (1.0–2.0) to each annotation; `activeTagFilter: String?` applies AND logic on top of state filter via `stateFilteredStairways` + `filteredStairways`; if AND yields zero, state filter is dropped and a toast fires; Filter button shows filled `brandAmber` icon when a tag filter is active; no walk-record action logic (all in sheet)
