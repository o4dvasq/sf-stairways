...
| `SeedDataService.swift` | Seeds preset `StairwayTag` records from `tags_preset.json` on first launch (key: `com.sfstairways.hasSeededTags`); deletes `WalkRecord` where `walked==false` once (key: `com.sfstairways.hasCleanedUnwalked`); `runTagDedupMigrationIfNeeded` deduplicates `StairwayTag` + `TagAssignment` and backfills `compoundKey` once (key: `hasRunTagDedupMigration_v1`); `deduplicateWalkRecordsIfNeeded` keeps the earliest `WalkRecord` per `stairwayID` and deletes CloudKit-sync duplicates once (key: `hasRunWalkRecordDedupMigration_v1`); `cleanupSeedBugRecordsIfNeeded` deletes the 8 ghost `WalkRecord` rows from the old `seedIfNeeded()` bug (walked=true, stairwayID in the 8 original targets, dateWalked 2026-03-09 or -10) once (key: `hasCleanedSeedBugRecords_v1`); iOS only — Mac inlines the same logic in `SFStairwaysMacApp` |
...
- `ConfettiView` (`Views/Components/ConfettiView.swift`) — celebration particle overlay; 60 particles randomized at `init` (xFraction, drift, fallSpeed, color, rotationRate, delay); `TimelineView(.animation)` + `Canvas` draws each frame by computing particle position from elapsed time; alpha fades to 0 as `y / height` approaches 1.0; host (`StairwayBottomSheet`) applies `.allowsHitTesting(false)` and controls show/hide via `showConfetti` state
