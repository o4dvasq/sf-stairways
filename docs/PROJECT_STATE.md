...
_Last updated: 2026-04-03 (sf-stairs-rebrand + celebration-bug + tag-pill-colors + walked-card-redesign pending)_
...
- **No HealthKit, no active walk recording** — "Mark Walked" is the only walk-logging action; tapping it fires a medium haptic, animates the sheet background to soft green (`surfaceWalked`), bounces the checkmark icon (driven by `celebrationTrigger` @State, not `isWalked`), and reveals a "N of M in [neighborhood]" progress line. The Hard Mode "Mark Anyway" path delays 0.3s after alert dismiss before firing celebration.
